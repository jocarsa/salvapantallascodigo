...
int main() {
    ...
    while (true) {
        ...
        // No frame buffer display - removed for faster calculation
        
        // Optional: Print progress every 60 seconds
        if (elapsed % 60 == 0 && elapsed > 0) {
            std::cout << "Progress: " << elapsed << " seconds / " << max_time << " seconds (" 
                      << (elapsed * 100 / max_time) << "%)" << std::endl;
        }
    }
